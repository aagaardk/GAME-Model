...
NODE_DATA <- read.delim("NA_NODE_LC2.txt",header= TRUE)
...
S <- readShapePoly("NorthAmeerica_20mi_grid_wAK_BPOP_NSmallard_join")
...
WSI_DATA <- read.delim("NorAm_historical_wsi_node_info.txt",header= TRUE)

#subset out a particular year
year = grep("2001",colnames(WSI_DATA),value=TRUE)
WSI_DATA.year = WSI_DATA[c("X_INDEX","Y_INDEX",grep("2001",colnames(WSI_DATA),value=TRUE))]
#Convert to moving_window format
WSI=data.table(WSI_DATA.year)
...
for (d in 1:5){
   for (i in 1:nrow(data_nodes) ) { #Loop through data nodes
    ...
    if (sum(bird_pop.am[d,focal_node_x-window_radius,focal_node_y-window_radius,],na.rm=TRUE) >0 ) {
      ...
      #Normalize probabily over window
