...
x = c("timeDate","lubridate","data.table","plyr","lattice","raster","rgeos",
      ...
      "geosphere","tgp","plotKML","animation","colorspace","colorRamps")
...
# NODE_LOCATIONS <- fread("node_locations.txt")
# 
# Add lat/long decimal degrees to NODE_DATA
# setkey(NODE_DATA,Y_INDEX,X_INDEX)
# setkey(NODE_LOCATIONS,Y_INDEX,X_INDEX)
# 
# Twenty_mi_nodes=NODE_LOCATIONS[NODE_DATA]
# setnames(Twenty_mi_nodes,c("LATITUDE","LONGITUDE","Long","Lat"),
#          c("LatDegree","LongDegree","LongUTM","LatUTM"))

# Save Twenty-mile Nodes
# work_dir=getwd()
# filename="/Twenty_mile_node_locations.csv"
# data_saver=file.path(paste(work_dir,filename,sep=""))
# write.csv(Twenty_mi_nodes,file=data_saver,row.names=F)
...
# Distance effects --------------------------------------------------------
hab_dist[,1]<-pnorm(node_move,NODE_DATA[,27],
                    sapply(NODE_DATA[,28],function(x) max(x,30)))
hab_dist[,2]<-pnorm(node_move,NODE_DATA[,29],
                    sapply(NODE_DATA[,30],function(x) max(x,30)))
hab_dist[,3]<-pnorm(node_move,NODE_DATA[,31],
                    sapply(NODE_DATA[,32],function(x) max(x,30)))
...
S.lat_long = merge(S, Twenty_mi_nodes)
...
#   )
# 
# SnowDepth.extent = NCEP.restrict(SnowDepth.extent,months2remove=6,
#                                  set2na=F)
# 
# AirDensity.extent =
#   NCEP.restrict(
...
# }
# close(pb)
# range(SnowDepth.resamp,na.rm=T)
# 
# AirDensity.resamp = 
#   array(
#     data = NA,
...
# # Air Temperature
# HistTemp.dt=Temp.dt[,round(mean(daily_temp),digits=3),
#                 by=.(date,NCEP_LATITUDE,NCEP_LONGITUDE)]
# setnames(HistTemp.dt,"V1","daily_temp")
# HistTemp.dt[,year:=substr(date,1,4)]
# HistTemp.dt=HistTemp.dt[,.(date,year,NCEP_LATITUDE,NCEP_LONGITUDE,daily_temp)]
# 
# daily_mean_temperature=dcast(HistTemp.dt,NCEP_LATITUDE+NCEP_LONGITUDE~date,
#                              value.var='daily_temp')
# setkey(daily_mean_temperature,NCEP_LATITUDE,NCEP_LONGITUDE)
# 
# # Snow Depth
# HistSnow.dt=Snow.dt[,round(mean(daily_snow),digits=3),
#                     by=.(date,NCEP_LATITUDE,NCEP_LONGITUDE)]
# setnames(HistSnow.dt,"V1","daily_snow")
# HistSnow.dt[,year:=substr(date,1,4)]
# HistSnow.dt=HistSnow.dt[,.(date,year,NCEP_LATITUDE,NCEP_LONGITUDE,daily_snow)]
# 
# daily_mean_snow=dcast(HistSnow.dt,NCEP_LATITUDE+NCEP_LONGITUDE~date,
...
# Twenty_mi_nodes_strpd=Twenty_mi_nodes[,.(Y_INDEX,X_INDEX,code,LatDegree,
#                                          LongDegree)]
# setnames(Twenty_mi_nodes_strpd, old=colnames(Twenty_mi_nodes_strpd),
#          new = c("Y_INDEX","X_INDEX","NODE","TMN_LATITUDE","TMN_LONGITUDE"))
# Twenty_mi_nodes_strpd[,TMN_LONGITUDE:=TMN_LONGITUDE+360]
# 
# node_locations = Twenty_mi_nodes_strpd[,.(TMN_LONGITUDE,TMN_LATITUDE)]
# node_locations=as.matrix(node_locations,nrow=nrow(Twenty_mi_nodes),ncol=2)
# 
# # Air Density
# HistAirDensity.dt = 
#   AirDensity.dt[
...
# 
# # Air Temperature
# samp_locations = HistTemp.dt[,.(NCEP_LONGITUDE,NCEP_LATITUDE)]
# samp_locations = unique(samp_locations)
# samp_locations=as.matrix(samp_locations,nrow=nrow(HistTemp.dt),ncol=2)
# 
...
# 
# TMN_daily_mean_temperature = matrix(0,nrow=nrow(closest_samp),
#                                     ncol=ncol(daily_mean_temperature))
# 
# TMN_daily_mean_temperature = data.table(TMN_daily_mean_temperature)
# 
...
# 
# # Snow Depth
# samp_locations = HistSnow.dt[,.(NCEP_LONGITUDE,NCEP_LATITUDE)]
# samp_locations = unique(samp_locations)
# samp_locations=as.matrix(samp_locations,nrow=nrow(HistSnow.dt),ncol=2)
# 
# closest_samp=matrix(NA,nrow=nrow(node_locations),ncol=4)
# closest_samp[,1:2]=node_locations
# for(i in 1:nrow(node_locations)){
...
# closest_samp=data.table(closest_samp)
# 
# TMN_daily_mean_snow = matrix(0,nrow=nrow(closest_samp),
#                              ncol=ncol(daily_mean_snow))
# 
...
#             paste(getwd(), "/NorAm_historical_snow_node_info.txt",
#                   sep=""), sep="\t", col.names=T, row.names=F)
# 
# # Air Density
# write.table(
...
# Read in files for diagnostics for wrapper -------------------------------
{
  ...
  # # in a given node on a given day, using data form 1957 to 2014. This can be 
  # # used for a static representation of temprature
  # #
  # # 2) HISTORICAL: Any individual year (1957 to 2014) can be used to set the 
  ...
  # # 
  # # year_of_interest = "2014" #must be a character
  # # day_of_interest = "12_01"  #must be a character
  # # 
...
setnames(Temp.dt,names(Temp.dt),c("date","NCEP_LATITUDE","NCEP_LONGITUDE",
                                  "daily_temp"))
Temp.dt[,date:=substr(date,1,10)]
Temp.dt=Temp.dt[,round(mean(daily_temp),digits=3),
                by=.(date,NCEP_LATITUDE,NCEP_LONGITUDE)]
setnames(Temp.dt,"V1","daily_temp")

Snow.dt = fread("NA_snowdepth_data.txt")
setnames(Snow.dt,names(Snow.dt),c("date","NCEP_LATITUDE","NCEP_LONGITUDE",
                                  "daily_snow"))
Snow.dt[,date:=substr(date,1,10)]
Snow.dt=Snow.dt[,round(mean(daily_snow),digits=3),
                by=.(date,NCEP_LATITUDE,NCEP_LONGITUDE)]
setnames(Snow.dt,"V1","daily_snow")
Snow.dt[,daily_snow:=ifelse(daily_snow<0,0,daily_snow)]

Weather.dt = Temp.dt[,.(date,NCEP_LATITUDE,NCEP_LONGITUDE)]
Weather.dt[,`:=`(daily_snow_score=(Snow.dt[,((daily_snow*257)/400)])*0.394,
                 daily_temp_score=-(Temp.dt[,daily_temp]),
                 year=substr(date,1,4))]

# Calculate temp_day and snow_day, from Schummer et al. 2010
Weather.dt[,`:=`(cold=ifelse(Temp.dt[,daily_temp]<0,1,0),
                 deep=ifelse(Snow.dt[,daily_snow]>=2.54,1,0))]
Weather.dt[,`:=`(temp_day=sequence(rle(cold)$lengths) * cold,
                 snow_day=sequence(rle(deep)$lengths) * deep),by=.(year)]
# 
# Calculate WSI -----------------------------------------------------------
WSI_data=Weather.dt[,.(daily_temp_score,temp_day,daily_snow_score,snow_day)]
WSI_data[,`:=`(WSI=rowSums(WSI_data))]

Weather.dt[,`:=`(WSI=WSI_data[,WSI])]
Weather.dt = Weather.dt[,.(date,NCEP_LATITUDE,NCEP_LONGITUDE,daily_temp_score,
                           daily_snow_score,temp_day,snow_day,WSI)]

WSI.dt = Weather.dt[,.(date,NCEP_LATITUDE,NCEP_LONGITUDE,WSI)]

# Historical WSI per node
HistWSI.dt=WSI.dt
setnames(HistWSI.dt,"WSI","daily_wsi")

daily_mean_wsi=dcast(HistWSI.dt,NCEP_LATITUDE+NCEP_LONGITUDE~date,
                      value.var='daily_wsi')
setkey(daily_mean_wsi,NCEP_LATITUDE,NCEP_LONGITUDE)
...
node_locations = Twenty_mi_nodes_strpd[,.(TMN_LONGITUDE,TMN_LATITUDE)]
samp_locations = HistWSI.dt[,.(NCEP_LONGITUDE,NCEP_LATITUDE)]
samp_locations = unique(samp_locations)

node_locations=as.matrix(node_locations,nrow=nrow(Twenty_mi_nodes),ncol=2)
samp_locations=as.matrix(samp_locations,nrow=nrow(HistWSI.dt),ncol=2)
...
TMN_daily_mean_wsi = matrix(0,nrow=nrow(closest_samp),
                             ncol=ncol(daily_mean_wsi))

TMN_daily_mean_wsi = data.table(TMN_daily_mean_wsi)
...
write.table(TMN_daily_mean_wsi,
            paste(getwd(),"/NorAm_historical_wsi_node_info.txt",
                  sep=""),sep="\t",col.names=T,row.names=F)
# 
# # Average WSI per node
# WSI.dt[,date:=substr(date,6,10)]
# MeanWSI.dt=WSI.dt[,round(mean(WSI),digits=3),
#                     by=.(date,NCEP_LATITUDE,NCEP_LONGITUDE)]
# setnames(MeanWSI.dt,"V1","daily_wsi")
# MeanWSI.dt=MeanWSI.dt[,.(date,NCEP_LATITUDE,NCEP_LONGITUDE,daily_wsi)]
# 
# daily_mean_wsi=
#   dcast(MeanWSI.dt,NCEP_LATITUDE+NCEP_LONGITUDE~date,
#         value.var='daily_wsi')
# setkey(daily_mean_wsi,NCEP_LATITUDE,NCEP_LONGITUDE)
# 
# #Define nodes by wsi based on distance from node centroid to
...
# 
# node_locations = Twenty_mi_nodes_strpd[,.(TMN_LONGITUDE,TMN_LATITUDE)]
# samp_locations = MeanWSI.dt[,.(NCEP_LONGITUDE,NCEP_LATITUDE)]
# samp_locations = unique(samp_locations)
# 
# node_locations=as.matrix(node_locations,nrow=nrow(Twenty_mi_nodes),ncol=2)
# samp_locations=as.matrix(samp_locations,nrow=nrow(MeanWSI.dt),ncol=2)
# 
# closest_samp=matrix(NA,nrow=nrow(node_locations),ncol=4)
# closest_samp[,1:2]=node_locations
# for(i in 1:nrow(node_locations)){
