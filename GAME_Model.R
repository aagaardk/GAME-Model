...
#                             with=F]))
#     
#     BC_Abund_Next = (BM_PopDist + (BC_Abund_Move/sum(BC_Abund_Move))) / 2
#       
#     BC_Abund_New = paste0("BodyMass_dist_discrete_day",i)
...
availhabitat = WSI_availhabitat = vector('list')
mortality = annmean_availhabitat = annsd_availhabitat =
  annmin_availhabitat = median_forage = vector()
for(i in 1:(length(year_from_record)-1)){
...
norm_meanWSI_availhabitat = 
  ZO_norm(unlist(lapply(WSI_availhabitat,mean)))
...
SummaryStats =
  data.table(Years=year_from_record[1:(length(year_from_record)-1)],
             ...
             MeanWSI_AvailHab=unlist(lapply(WSI_availhabitat,mean)),
             NormMeanWSI_AvailHab=norm_meanWSI_availhabitat)
...
Combined_Plot =
  ...
  geom_point(aes(x=Years,y=NormAnnMinHab),size=2,col="blue") +
  geom_line(aes(x=Years,y=NormAnnMinHab),lty="dotted",col="blue") +
  xlab("Years") +
...
Norm_Mort_MeanHab_Plot =
  ...
  ylab("Normalized Mortality") +
  theme_minimal()
ggsave(paste0("Norm_Mortality_v_ProporMeanAnnAvailHab.jpeg"),
...
CoM_DailyDist = CoM_DailyDir = CenterofMass_coords =
  vector('list')
SumCoM_AdjDist = MeanCoM_AdjDist = CoM_NS_Dist = vector()
for(i in 1:(length(year_from_record)-1)){
  COMPLETE_DATA = fread(paste0(year_from_record[i],"_Prediction.txt"))
  ...
  SumCoM_AdjDist[i] = sum(CoM_DailyDist[[i]])
  
  MeanCoM_AdjDist[i] = mean(CoM_DailyDist[[i]])
...
DistanceMetric = data.table(do.call(cbind,CoM_DailyDist))
setnames(DistanceMetric,names(DistanceMetric),
         c(paste0("Distance_",
...
MovementMetrics = cbind(DayofPeriod=days[-which.max(days)],
                        DistanceMetric,DirectionMetric)
...
for(i in 1:(length(year_from_record)-1)){
  CenterofMass_coords[[i]][,`:=`(
    ...
    MeanWSI=WSI_availhabitat[[i]],
    Mean_AvailHab=availhabitat[[i]],
    Norm_AvailHab=ZO_norm(unlist(availhabitat[[i]])))]
}
...
CenterofMass_coords[,`:=`(
  ColorGradient=col_pal(57)[as.numeric(cut(MeanWSI,breaks = 10))],
...
LatAbundTime_plot =
  ...
  geom_line(stat='smooth',method='loess',se=FALSE,alpha=0.6) +
  scale_colour_gradient(low='gray90',high='black') +
  xlab("Day of Nonbreeding Period") +
  ylab("Most Populace Latitude") +
  theme_minimal()
...
MPML_WSI = 
  CenterofMass_coords[,.(min(coords.x2),min(MeanWSI),
                         unique(Mean_AvailHab)),by=Year]
setnames(MPML_WSI,names(MPML_WSI),c('Year','Latitude','WSI',
                                    'MeanAvailableHabitat'))
...
MPML_Year_plot =
  ...
  xlab("Year") +
  ylab("Southern-most Most Populace Latitude") +
  theme_minimal()
...
MPML_WSI_plot =
  ggplot(data=MPML_WSI,
         aes(x=WSI,y=Latitude)) +
  geom_point() +
  geom_smooth(method='glm') +
  scale_x_continuous(limits=c(-8.55,-7.5)) +
  xlab("WSI") +
  ylab("Southern-most Most Populace Latitude") +
  theme_minimal()
...
Year_WSI_plot =
  ...
  geom_point() +
  geom_smooth(method='glm') +
  scale_y_continuous(limits=c(-8.55,-7.5)) +
  xlab("Year") +
  ylab("WSI") +
  theme_minimal()
...
SummaryStats[,`:=`(TotaDist_Traveled=SumCoM_AdjDist,
                   MeanDist_Between=MeanCoM_AdjDist,
...
dev.new()
tiff(file='Aggregate_MigrationRoute_CoM_STPRVBRDR.tiff',
     bg="transparent",width=1500,height=900)
par(mar=c(0,0,0,0),oma=c(0,0,0,0))
plot_colors = adjustcolor('gray',alpha.f=0.1)
