...
x = c('compiler','fields','maptools','data.table','plyr','foreign',
      ...
      'ggtern','doSNOW','doParallel','bit64','piecewiseSEM')
...
# Integration function for beta distribution
integrate_beta = 
  Vectorize(
    function(
      l,
...
FDR_func = 
  function(
    ...
    Empir_coeff * basalmetabolicrate
  }
...
#   Temp_WSI_AirDensityDT_NonbreedingPeriod = 
#     TempDT_NonbreedingPeriod[
#       WSIDT_NonbreedingPeriod[
#         AirDensityDT_NonbreedingPeriod
#         ]
...
body_mass_lower = 
  body_mass_norm[
    1:length(body_mass_norm)-1
    ]

body_mass_upper = 
  body_mass_norm[
    2:length(body_mass_norm)
    ]
...
# as its source exclusively, and not all of the 13% of body mass comprised of lipids is
# available for powered flight processes. We set this to 10%.
BodyCondition_Table = 
  data.table(
    BC_bins = c(1, bc_bins + 1),
...
#     ,
#     lm(
#       flight_cost_kgkm ~ body_mass_g
#     )
#     ]$coefficients

# The above table may take a while to read in if enough simulations are run, 
# so here are the values that comes out from the regressions (which also take 
# a while to run):
flight_vel_body_mass_rel =
  c(
    60.96942514559717807,
    0.01917621425459612
  )
  
flight_cost_body_mass_rel =
  c(
    -1.425342860140741e-05,
    6.970106259119213e-08 
  )
...
# # Trying to implement the following does not seem practical: no 
# # empirical relation of air pressure on relative abundance of mallards. 
# # In fact, see: 
...
# 
# # Probability of departure component related to air density
# ADProb = 
#   function(
#     x
#   ) {
...
# Set values for Cobb-Douglass function exponents
W_Forage = 0.55
W_Roost = 0.25
W_Breed = 0.8
W_AirDens = 0.4
  ...
  data.table(
    ...
    ForageAvailComponent = 
      (
        (
          (
            (-inflection_seq ^ 3) * W_Forage
          ) +
            (max(inflection_seq) ^ 3) * W_Forage
        ) + 0.03
      ),
    AirDensityComponent = 
      (
        (
          (
            (-inflection_seq ^ 3) * W_AirDens
          ) +
            (max(inflection_seq) ^ 3) * W_AirDens
        ) + 0.03
      ),
    RoostingHabComponent = 
      (
        (
          (
            (-inflection_seq ^ 3) * W_Roost
          ) +
            (max(inflection_seq) ^ 3) * W_Roost
        ) + 0.03
      ),
    BreedingDistComponent = 
      (
        (
          (inflection_seq ^ 3) * W_Breed
        ) + 0.03
      ),
    GammaMoveComponent = 
      W_Gamma + 0.01
...
foreach(
  ...
  .verbose = T) %dopar% {
    ...
    for(i in 1:length(days)) {
      ...
      # Determine WSI-, Air density-, and body condition-dependent 
...
focal_years = c(1957,1962, 1989, 1999, 2012)
# for(i in 1:(length(year_from_record)-1)){
for(i in 1:length(focal_years)) {
  # COMPLETE_DATA =
  #   fread(
  #     paste0(
  #       getwd(),
  ...
  for(j in 1:length(days)){
    daily_mort[[i]][j] = 
      COMPLETE_DATA[,sum(get(paste0('N_Abund_',j-1)))] - 
      COMPLETE_DATA[,sum(get(paste0('N_Abund_',j)))]
    print((j / length(days)) * 100)
  }
  ...
  print((i / length(focal_years)) * 100)
}

# norm_mortality = ZO_std(mortality)
# norm_autumn_mortality = ZO_std(autumn_mortality)
# norm_winter_mortality = ZO_std(winter_mortality)
# norm_spring_mortality = ZO_std(spring_mortality)
# norm_annmean_availhabitat = ZO_std(annmean_availhabitat)
# norm_annmin_availhabitat = ZO_std(annmin_availhabitat)
# norm_AnnMeanWSI_availhabitat =
#   ZO_std(unlist(lapply(MeanWSI_availhabitat,mean)))
# norm_AnnMedianWSI_availhabitat =
#   ZO_std(unlist(lapply(MedianWSI_availhabitat,mean)))
# norm_AnnMeanWSI_allhabitat =
#   ZO_std(unlist(lapply(MeanWSI_allhabitat,mean)))
# norm_AnnMedianWSI_allhabitat =
#   ZO_std(unlist(lapply(MedianWSI_allhabitat,mean)))
# 
# SummaryStats =
...
# dev.off()
# 
annualavailhabitat_dt = data.table(annualavailhabitat)
setnames(annualavailhabitat_dt,
         names(annualavailhabitat_dt),
         c('MigDay',
           paste0('AvailHab_',
                  focal_years[1:(length(focal_years))])))

annualavailhabitat_melt = 
  melt(annualavailhabitat_dt,
       id.vars='MigDay',
       variable.name='Year',
       value='ProporAvailHab')
annualavailhabitat_melt[,Year:=gsub('AvailHab_','',Year)]
setkey(annualavailhabitat_melt,MigDay,Year)
...
setnames(daily_mortality,
         names(daily_mortality),
         paste0('Mortality_',focal_years)
)
MigDay = 1:length(days)
daily_mortality = 
  cbind(
    MigDay,
    daily_mortality
...
dailymortality_melt = 
  melt(daily_mortality,
       id.vars='MigDay',
       variable.name='Year',
       value='DailyMortality')
dailymortality_melt[,Year:=gsub('Mortality_','',Year)]
setkey(dailymortality_melt,MigDay,Year)

annualavailhabitat_dailymortality_melt = 
  dailymortality_melt[annualavailhabitat_melt]

annualavailhabitat_dailymortality_melt[,Extreme:=
                                         ifelse(
                                           Year %like% 1957 |
                                             Year %like% 1962 |
                                             Year %like% 1989,
...
annualavailhabitat_dailymortality_melt[,DailyMortality:=
                                         1-ZO_std(DailyMortality)]

annualavailhabitat_dailymortality_melt_dt = 
  melt(annualavailhabitat_dailymortality_melt,
       id.vars=c('MigDay','Year','Extreme'),
       measure.vars=c('DailyMortality','ProporAvailHab'),
       variable.name='Variable',
       value.name='Normalized_Value')

# Annual_AvailHab_Plot =
#   ggplot(data=annualavailhabitat_melt,
...
#   theme_minimal()
Annual_AvailHab_Plot =
  ggplot(data=annualavailhabitat_dailymortality_melt_dt,
         aes(x=MigDay,y=Normalized_Value,linetype=Variable)) +
  geom_line(lwd = 1) +
  facet_wrap(~Year) +
  xlab("Day of Non-breeding Period") +
  ylab("Normalized Mean Value (Annual Available Habitat or Daily Mortality") +
  theme_minimal()
Annual_AvailHab_Plot
ggsave(paste0("ProporAnnAvailHab_Mortality_OverTime_extremes.jpeg"),
       Annual_AvailHab_Plot,width=10,height=5)
dev.off()

# Annual_AvailHab_Plot_smoothed =
#   ggplot(data=annualavailhabitat_melt,
#          aes(x=MigDay,y=ProporAvailHab,group=Year,linetype=Extreme)) +
#   geom_smooth(lwd = 1) +
#   scale_y_continuous(breaks=c(0,0.2,0.4,0.6,0.8,1)) +
#   xlab("Day of Non-breeding Period") +
#   ylab("Normalized Mean Annual Available Habitat") +
#   theme_minimal()
# Annual_AvailHab_Plot_smoothed
# # ggsave(paste0("ProporAnnAvailHab_OverTime_smoothed.jpeg"),
# #        Annual_AvailHab_Plot_smoothed,width=5,height=5)
# # dev.off()
...
#     Norm_AvailHab=as.vector(ZO_std(unlist(availhabitat[[i]]))))]
# }
# 
...
DayAbund = 
  ZO_std(as.matrix(COMPLETE_DATA[,N_Abund_0]))
...
for(j in 1:(length(year_from_record)-1)) {
  ...
  DailyAbundAnimate_func = function() {
    for(i in 1:length(days)){
      ...
      start_timer = Sys.time()
      DayAbund = 
        ZO_std(as.matrix(COMPLETE_DATA[,paste0("N_Abund_",i-1),with=F]))
