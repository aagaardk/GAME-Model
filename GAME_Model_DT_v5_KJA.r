...
#   NODE_DATA_TEMP = NODE_DATA[MetCostDT_NonbreedingPeriod]
# 
#   NODE_DATA_TEMP[,`:=`(HarvestDisturb = 0,
#                        FDR = DailyMetabolizableEnergy)]
# 
#   InCols = names(NODE_DATA_TEMP)[which(names(NODE_DATA_TEMP)
#                                        %like% "TD_BMR_day_")]
#   OutCols = paste0("DailyGain_day",1:num_days)
#   NODE_DATA_TEMP[,c(OutCols) :=
...
BodyCondition_Table[length(bc_bins),MaxLike_flightdist:=flight_dist]
...
for(i in 1:length(bc_bins)){
  BC_dist_seq_ls[[i]] = 
    rep(BC_distprob[i],(((flightdist_distrib[i]*flight_dist)+1))/
          node_size)
}
...
BC_transition_g_mtx = rbind(BC_transition_g_mtx,
                            rep(0,length(bc_bins)-1))
BC_transition_g_mtx = cbind(rep(0,length(bc_bins)),
                            BC_transition_g_mtx)

BC_transitions = 
  sort(BC_transition_g_mtx[,length(bc_bins)][
    which(BC_transition_g_mtx[,length(bc_bins)]>0)])
...
#   ylim(0,1) +
#   labs(x="Day of Non-breeding Period",y="Relative weight") +
#   theme_minimal() +
#   theme(
...
for(i in 1:(length(year_from_record)-1)){  
  ...
  COMPLETE_DATA[!(most_populace_node),within_populace:=0]
  propor_abund = (rowSums(COMPLETE_DATA[,.SD,.SDcols=target_cols]) *
    COMPLETE_DATA[,within_populace])
  COMPLETE_DATA[,mig_track:=ZO_norm(propor_abund)]
  
  MostPopulaceNode_ProporPlot = 
    ggplot(data=COMPLETE_DATA,aes(x=X_INDEX,y=Y_INDEX,col=mig_track)) +
    geom_point(size=2,shape=15) +
    scale_color_gradientn(
      name=paste0(">98% Nodes\n"),
  ...
  ggsave(paste0("MostPopulaceNodes_Propor",year_from_record[i],".jpeg"),
         MostPopulaceNode_ProporPlot,width=5,height=5)
  
  MostPopulaceNode_BinaryPlot = 
    ggplot(data=COMPLETE_DATA,
           aes(x=X_INDEX,y=Y_INDEX,col=within_populace)) +
    geom_point(size=2,shape=15) +
    theme_minimal() +
    guides(colour=FALSE) +
    theme(
  ...
  ggsave(paste0("MostPopulaceNodes_Binary",year_from_record[i],".jpeg"),
         MostPopulaceNode_BinaryPlot,width=5,height=5)
...
# # World map
# world_data = data.table(map_data("world"))
# NorthAmerica_data = 
#   world_data[region=="USA"|region=="Canada"]
# NorthAmerica_data = NorthAmerica_data[long<(-50)&long>(-137)]
# NorthAmerica_data[,`:=`(norm_long = ZO_norm(long),
#                         norm_lat = ZO_norm(lat))]
# 
# ggplot()
# 
# NODE_DATA[,`:=`(norm_XINDEX = ZO_norm(X_INDEX),
#                 norm_YINDEX = ZO_norm(Y_INDEX),
#                 norm_N_0 = ZO_norm(N_Abund_0))]
# 
# test = ggplot() +
#   geom_polygon(data=NorthAmerica_data,
#                aes(x=norm_long,y=norm_lat,group=group))
# 
# test2 = test + geom_point(data=NODE_DATA,
#                           aes(x=norm_XINDEX,y=norm_YINDEX,
#                               col=norm_N_0),size=2,shape=15) +
#   scale_color_gradientn(
